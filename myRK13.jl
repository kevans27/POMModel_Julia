...
function myRK13(C, p, t)
    ...
    mom=C[2]; #mmolC.m-3
    z=C[3]; #m
    bo_pa=zeros(nbft); #mmolC.particle-1
    dbo_padt=zeros(nbft); #mmolC.particle-1.day-1
    dpomsdt=zeros(nbft); #mmolC.particle-1.day-1
    dCdt=zeros(3+nbft); #mmolC.particle-1.day-1
    vmom=zeros(nbft); #mmolC.m-3.day-1
